...
class P2BVoteNetRPN(nn.Module):
    ...
    def __init__(self, feature_channel, vote_channel=256, num_proposal=64, normalize_xyz=False): #从debug看，feature_channel=256
        super().__init__()
    ...
    def forward(self, xyz, feature):
        ...
        score = estimation_cla.sigmoid()#（1，128）

        xyz_feature = torch.cat((xyz.transpose(1, 2).contiguous(), feature), dim=1)#（1，259，128）

        offset = self.vote_layer(xyz_feature)#（1，259，128）
        vote = xyz_feature + offset#（1，259，128）
        vote_xyz = vote[:, 0:3, :].transpose(1, 2).contiguous()#(1,128,3)
        vote_feature = vote[:, 3:, :]#(1,256,128)

        vote_feature = torch.cat((score.unsqueeze(1), vote_feature), dim=1)#(1,257,128)

        center_xyzs, proposal_features = self.vote_aggregation(vote_xyz, vote_feature, self.num_proposal)#(1,64,3) (1,256,64)
        proposal_offsets = self.FC_proposal(proposal_features)#(1,5,64)
